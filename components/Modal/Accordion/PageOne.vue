...
<template>
  <div class="flex items-center gap-[30px]">
    ...
    >
      <AtomIcon name="arrow" :size="24" :class="{ 'text-pr-light': currentPage === 1 }" class="rotate-180 text-pr hover:text-pr-dark duration-300" />
    </div>
    <div v-if="data.lastPage <= 7" class="flex items-center gap-[18px] pb-1 md:gap-6">
      <div
        ...
        class="cursor-pointer items-center text-pr text-[26px] hover:text-pr-dark duration-300"
      >
        {{ index }}
      </div>
    </div>
    <div v-else class="flex items-center gap-[18px] pb-1 md:gap-6">
      ...
      <span v-show="currentPage > 4" class="items-center tracking-[3.64px] text-pr text-[26px]">...</span>
      <div
        @click="changePage(2)"
      ...
      <div v-show="currentPage < 5" @click="changePage(5)" class="cursor-pointer items-center text-pr text-[26px] hover:text-pr-dark duration-300">5</div>
      <!-- mid -->
      <div
      ...
      <span
        ...
        class="items-center tracking-[3.64px] text-pr text-[26px]"
        >...</span
      >
      <div
        :class="{
    ...
    <div
      ...
      class="cursor-pointer"
    >
      <AtomIcon name="arrow" :size="24" :class="{ 'text-pr-light': currentPage === data.lastPage }" class="text-pr hover:text-pr-dark duration-300" />
    </div>
  </div>
</template>
